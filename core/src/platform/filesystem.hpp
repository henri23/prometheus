...
VOLTRUM_API b8 filesystem_open(
    const char* path,
    File_Modes mode,
    b8 binary,
...
// EOF characters
VOLTRUM_API b8 filesystem_read_line(
    File_Handle* handle,
    char** line_buf);

VOLTRUM_API b8 filesystem_write_line(
    File_Handle* handle,
    const char* text);
...
VOLTRUM_API b8 filesystem_read(
    File_Handle* handle,
    u64 data_size,
    void* out_data,
    u64* out_bytes_read);
...
VOLTRUM_API b8 filesystem_read_all_bytes(
    File_Handle* handle,
    u8** out_bytes,
    u64* out_bytes_read);

VOLTRUM_API b8 filesystem_write(
    File_Handle* handle,
    u64 data_size,
    const void* data,
